...
1. **Flasher les scripts :**
   - `voiture_manette.py` sur la voiture (ESP32 côté robot)  
   - `manette.py` sur la manette (ESP32 côté télécommande)  

2. **Alimenter les deux modules ESP32**.

3. La voiture crée automatiquement un **réseau Wi-Fi** nommé `RobotVoiture`.  
   La manette s’y connecte et envoie les commandes.

4. Appuyez sur les boutons pour contrôler la voiture :  
   - ⬆️ Avancer  
   - ⬇️ Reculer  
   - ⬅️ Tourner à gauche  
   - ➡️ Tourner à droite  
...
5. Pour le **mode automatique**, flashez `Voiture.py` sur l’ESP32 de la voiture.
